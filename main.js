...
var init_geminiClient = __esm({
  "src/agents/geminiClient.ts"() {
    init_dist();
    GeminiClient = class _GeminiClient {
...
Retorne APENAS um objeto JSON v\xE1lido com a seguinte estrutura:
{
  "abstract": "Resumo executivo conciso do documento (m\xE1ximo 200 palavras)",
  "keyPoints": ["Ponto-chave 1", "Ponto-chave 2", "Ponto-chave 3"],
  "mainTopics": ["T\xF3pico principal 1", "T\xF3pico principal 2", "T\xF3pico principal 3"]
}

Instru\xE7\xF5es:
- O abstract deve ser um par\xE1grafo conciso que capture a ess\xEAncia do documento
- keyPoints devem ser os 3-5 pontos mais importantes do texto
- mainTopics devem ser os 3-5 temas principais abordados
- Use linguagem clara e objetiva
- Mantenha o foco no conte\xFAdo mais relevante
`;
        try {
...
Retorne APENAS um array JSON v\xE1lido com a seguinte estrutura:
[
  {
    "name": "Nome do conceito",
    "category": "person|place|theory|formula|term|other",
    "description": "Descri\xE7\xE3o clara e concisa do conceito",
...
- Categorize cada conceito apropriadamente:
  * person: pessoas, autores, figuras hist\xF3ricas
  * place: locais, pa\xEDses, institui\xE7\xF5es
  * theory: teorias, modelos, frameworks
  * formula: f\xF3rmulas matem\xE1ticas, equa\xE7\xF5es
  * term: termos t\xE9cnicos, defini\xE7\xF5es
  * other: outros conceitos importantes
- A descri\xE7\xE3o deve ser clara e informativa (1-2 frases)
- Avalie a import\xE2ncia baseada na relev\xE2ncia no texto
- Foque nos conceitos mais significativos e \xFAnicos
`;
        try {
          const result = await this.model.generateContent(prompt);
          const response = await result.response;
      ...
      async generateQA(text) {
        const prompt = `
Analise o seguinte texto e gere perguntas e respostas educativas baseadas no conte\xFAdo.
...
[
  {
    "question": "Pergunta clara e espec\xEDfica sobre o conte\xFAdo",
    "answer": "Resposta completa e informativa",
    "difficulty": "easy|medium|hard",
    "category": "Categoria tem\xE1tica da pergunta"
  }
]
...
- Varie a dificuldade: algumas b\xE1sicas, algumas intermedi\xE1rias, algumas avan\xE7adas
- As respostas devem ser completas mas concisas
- A categoria deve refletir o tema da pergunta (ex: "Conceitos B\xE1sicos", "Aplica\xE7\xE3o Pr\xE1tica", "An\xE1lise Cr\xEDtica")
- Foque em perguntas que ajudem na compreens\xE3o e revis\xE3o do material
- Evite perguntas muito \xF3bvias ou muito obscuras
`;
        try {
          const result = await this.model.generateContent(prompt);
          const response = await result.response;
...
Retorne APENAS um objeto JSON v\xE1lido com a seguinte estrutura:
{
  ...
  "structuralElements": {
    "sections": ["Se\xE7\xE3o 1", "Se\xE7\xE3o 2"],
    "keyTerms": ["termo1", "termo2"],
    "relationships": ["rela\xE7\xE3o1", "rela\xE7\xE3o2"]
  }
}

Instru\xE7\xF5es:
- Identifique se o texto cont\xE9m elementos temporais (timeline)
- Verifique se h\xE1 processos ou fluxos de trabalho (process)
- Detecte estruturas hier\xE1rquicas (hierarchy)
- Identifique compara\xE7\xF5es ou contrastes (comparison)
- Procure por dados estat\xEDsticos ou num\xE9ricos (statistics)
- Sugira tipos de diagramas Mermaid apropriados: flowchart, timeline, mindmap, pie, etc.
- Liste as principais se\xE7\xF5es ou divis\xF5es do texto
- Identifique termos-chave que aparecem frequentemente
- Detecte rela\xE7\xF5es importantes entre conceitos
`;
        try {
          const result = await this.model.generateContent(prompt);
          const response = await result.response;
      ...
      async testConnection() {
        try {
          const result = await this.model.generateContent('Responda apenas "OK" se voc\xEA conseguir me ouvir.');
          const response = await result.response;
      ...
      static async validateApiKey(apiKey) {
        try {
          const client = new _GeminiClient(apiKey);
...
var CodexSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "Configura\xE7\xF5es do Codex" });
    containerEl.createEl("h3", { text: "Configura\xE7\xE3o da API Gemini" });
    new import_obsidian.Setting(containerEl).setName("Chave da API Gemini").setDesc("Insira sua chave da API do Google Gemini. Voc\xEA pode obter uma em https://makersuite.google.com/app/apikey").addText((text) => text.setPlaceholder("Insira sua chave da API").setValue(this.plugin.settings.geminiApiKey).onChange(async (value) => {
      this.plugin.settings.geminiApiKey = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Modelo Gemini").setDesc("Escolha o modelo Gemini a ser usado para processamento").addDropdown((dropdown) => dropdown.addOption("gemini-2.5-flash", "Gemini 2.5 Flash (Recomendado)").addOption("gemini-2.5-pro", "Gemini 2.5 Pro (Mais Avan\xE7ado)").setValue(this.plugin.settings.geminiModel).onChange(async (value) => {
      this.plugin.settings.geminiModel = value;
      await this.plugin.saveSettings();
    }));
    containerEl.createEl("h3", { text: "Configura\xE7\xF5es de OCR" });
    new import_obsidian.Setting(containerEl).setName("Habilitar OCR").setDesc("Ativar reconhecimento \xF3ptico de caracteres para PDFs escaneados").addToggle((toggle) => toggle.setValue(this.plugin.settings.enableOCR).onChange(async (value) => {
      this.plugin.settings.enableOCR = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Provedor de OCR").setDesc("Escolha entre processamento local (Tesseract) ou servi\xE7os em nuvem").addDropdown((dropdown) => dropdown.addOption("tesseract", "Tesseract (Local)").addOption("cloud", "Servi\xE7os em Nuvem").setValue(this.plugin.settings.ocrProvider).onChange(async (value) => {
      this.plugin.settings.ocrProvider = value;
      await this.plugin.saveSettings();
    }));
    containerEl.createEl("h3", { text: "Configura\xE7\xF5es de Sa\xEDda" });
    new import_obsidian.Setting(containerEl).setName("Pasta de Sa\xEDda").setDesc("Pasta onde as notas geradas ser\xE3o salvas").addText((text) => text.setPlaceholder("Codex Generated Notes").setValue(this.plugin.settings.outputFolder).onChange(async (value) => {
      this.plugin.settings.outputFolder = value;
      await this.plugin.saveSettings();
    }));
    containerEl.createEl("h3", { text: "Conte\xFAdo das Notas" });
    new import_obsidian.Setting(containerEl).setName("Incluir Resumo").setDesc("Gerar resumo do documento").addToggle((toggle) => toggle.setValue(this.plugin.settings.includeSummary).onChange(async (value) => {
      this.plugin.settings.includeSummary = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Incluir Conceitos-Chave").setDesc("Extrair e listar conceitos importantes").addToggle((toggle) => toggle.setValue(this.plugin.settings.includeConcepts).onChange(async (value) => {
      this.plugin.settings.includeConcepts = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Incluir Perguntas e Respostas").setDesc("Gerar perguntas e respostas baseadas no conte\xFAdo").addToggle((toggle) => toggle.setValue(this.plugin.settings.includeQA).onChange(async (value) => {
      this.plugin.settings.includeQA = value;
      await this.plugin.saveSettings();
    }));
    containerEl.createEl("h3", { text: "Formata\xE7\xE3o Avan\xE7ada" });
    new import_obsidian.Setting(containerEl).setName("Incluir Diagramas Mermaid").setDesc("Gerar diagramas Mermaid quando apropriado").addToggle((toggle) => toggle.setValue(this.plugin.settings.includeMermaidDiagrams).onChange(async (value) => {
      this.plugin.settings.includeMermaidDiagrams = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Incluir F\xF3rmulas LaTeX").setDesc("Formatar equa\xE7\xF5es matem\xE1ticas em LaTeX").addToggle((toggle) => toggle.setValue(this.plugin.settings.includeLatex).onChange(async (value) => {
      this.plugin.settings.includeLatex = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Incluir \xCDcones").setDesc("Adicionar \xEDcones \xE0s notas (requer plugin Iconize)").addToggle((toggle) => toggle.setValue(this.plugin.settings.includeIcons).onChange(async (value) => {
...
var ProgressModal = class extends import_obsidian2.Modal {
  ...
  createStageIndicators(container) {
    const stages = [
      { id: "ingestion", label: "Extra\xE7\xE3o", icon: "\u{1F4C4}" },
      { id: "analysis", label: "An\xE1lise IA", icon: "\u{1F9E0}" },
      { id: "generation", label: "Gera\xE7\xE3o", icon: "\u270D\uFE0F" },
      { id: "complete", label: "Conclu\xEDdo", icon: "\u2705" }
    ];
  ...
  updateProgress(progress) {
    ...
    const stageLabels = {
      ...
      "analysis": "Analisando conte\xFAdo com IA",
      "generation": "Gerando nota estruturada",
      "complete": "Processamento conclu\xEDdo",
  ...
  showCompletionState() {
    const cancelButton = this.contentEl.querySelector(".codex-cancel-button");
    if (cancelButton) {
...
var PDFNetworkStreamRangeRequestReader = class {
  ...
  _onError(status) {
    var _a2;
    (_a2 = this._storedError) != null ? _a2 : this._storedError = createResponseStatusError(status, this._url);
    for (const requestCapability of this._requests) {
      requestCapability.reject(this._storedError);
    }
    this._requests.length = 0;
...
processItems_fn = function(items) {
  ...
  (_c = (_b = __privateGet(this, _layoutTextParams)).ctx) != null ? _c : _b.ctx = __privateMethod(_a2 = _TextLayer, _TextLayer_static, getCtx_fn).call(_a2, __privateGet(this, _lang));
...
var _DrawingEditor = class _DrawingEditor extends AnnotationEditor {
  ...
  static startDrawing(parent, uiManager, _isLTR3, event) {
    ...
    (_a2 = __privateGet(_DrawingEditor, _currentPointerType)) != null ? _a2 : __privateSet(_DrawingEditor, _currentPointerType, pointerType);
    ...
    (_b = uiManager._editorUndoBar) == null ? void 0 : _b.hide();
...
var _AnnotationEditorLayer = class _AnnotationEditorLayer {
  ...
  async deserialize(data) {
    var _a2, _b;
    return await ((_b = __privateGet(_AnnotationEditorLayer, _editorTypes2).get((_a2 = data.annotationType) != null ? _a2 : data.annotationEditorType)) == null ? void 0 : _b.deserialize(data, this, __privateGet(this, _uiManager3))) || null;
  }
...
var IngestionAgent = class {
  async extractPDFContent(arrayBuffer) {
    try {
      const loadingTask = __webpack_exports__getDocument({ data: arrayBuffer });
      ...
      console.error("Erro ao extrair conte\xFAdo do PDF:", error);
      throw new Error(`Falha na extra\xE7\xE3o do PDF: ${error.message}`);
    }
  }
  async extractMetadata(pdf) {
    try {
      const metadata = await pdf.getMetadata();
      const info2 = metadata.info;
      return {
        title: (info2 == null ? void 0 : info2.Title) || void 0,
        author: (info2 == null ? void 0 : info2.Author) || void 0,
        subject: (info2 == null ? void 0 : info2.Subject) || void 0,
        creator: (info2 == null ? void 0 : info2.Creator) || void 0,
        producer: (info2 == null ? void 0 : info2.Producer) || void 0,
        creationDate: (info2 == null ? void 0 : info2.CreationDate) ? new Date(info2.CreationDate) : void 0,
        modificationDate: (info2 == null ? void 0 : info2.ModDate) ? new Date(info2.ModDate) : void 0,
        pages: 0
  ...
  cleanText(text) {
    return text.replace(/\n{3,}/g, "\n\n").replace(/[ \t]+/g, " ").replace(/^[ \t]+|[ \t]+$/gm, "").replace(/^\s*\n/gm, "").trim();
  }
  // Método para validar se o arquivo é um PDF válido
  static async validatePDF(arrayBuffer) {
    try {
      const loadingTask = __webpack_exports__getDocument({ data: arrayBuffer });
      const pdf = await loadingTask.promise;
  ...
  static async getPDFInfo(arrayBuffer) {
    try {
      const loadingTask = __webpack_exports__getDocument({ data: arrayBuffer });
      const pdf = await loadingTask.promise;
      const metadata = await pdf.getMetadata();
      ...
      throw new Error(`Erro ao obter informa\xE7\xF5es do PDF: ${error.message}`);
    }
  }
  // Método para extrair texto de páginas específicas (útil para PDFs grandes)
  async extractPageRange(arrayBuffer, startPage, endPage) {
    try {
      const loadingTask = __webpack_exports__getDocument({ data: arrayBuffer });
      const pdf = await loadingTask.promise;
      let text = "";
      ...
      throw new Error(`Erro ao extrair p\xE1ginas ${startPage}-${endPage}: ${error.message}`);
    }
  }
};

// src/ui/fileSelector.ts
var FileSelectorModal = class extends import_obsidian3.Modal {
  ...
  setupDropZone(dropZone) {
    ...
    dropZone.addEventListener("drop", (e) => {
      ...
      if (pdfFile) {
        ...
        new import_obsidian3.Notice("Por favor, selecione um arquivo PDF v\xE1lido.");
      }
    });
  ...
  async createVaultPDFList(container) {
    ...
    for (const file of pdfFiles.slice(0, 10)) {
      ...
      try {
        ...
        const size = this.formatFileSize((stat == null ? void 0 : stat.size) || 0);
        const date = new Date((stat == null ? void 0 : stat.mtime) || 0).toLocaleDateString("pt-BR");
        fileDetails.textContent = `${size} \u2022 ${date}`;
      } catch (error) {
        fileDetails.textContent = "Informa\xE7\xF5es n\xE3o dispon\xEDveis";
      }
  ...
  async handleFileSelection(file) {
    ...
    try {
      ...
      new import_obsidian3.Notice("Erro ao validar o arquivo PDF.");
      return;
    }
    if (!isValid) {
      new import_obsidian3.Notice("O arquivo selecionado n\xE3o \xE9 um PDF v\xE1lido.");
      return;
    }
  ...
  async showFilePreview(file) {
    ...
    previewContainer.innerHTML = `
      ...
      <div class="codex-preview-content">
        <div class="codex-preview-loading">Carregando informa\xE7\xF5es...</div>
      </div>
    `;
    try {
      let arrayBuffer;
      ...
      const info2 = await IngestionAgent.getPDFInfo(arrayBuffer);
      const previewContent = previewContainer.querySelector(".codex-preview-content");
      previewContent.innerHTML = `
        <div class="codex-preview-info">
          ...
          <div class="codex-info-item">
            <span class="codex-info-label">P\xE1ginas:</span>
            <span class="codex-info-value">${info2.pages}</span>
          </div>
          ${info2.title ? `
            <div class="codex-info-item">
              <span class="codex-info-label">T\xEDtulo:</span>
              <span class="codex-info-value">${info2.title}</span>
            </div>
          ` : ""}
          ${info2.author ? `
            <div class="codex-info-item">
              <span class="codex-info-label">Autor:</span>
              <span class="codex-info-value">${info2.author}</span>
            </div>
          ` : ""}
        </div>
      ...
      `;
      const confirmBtn = previewContent.querySelector(".codex-confirm-btn");
      const backBtn = previewContent.querySelector(".codex-back-btn");
      ...
      previewContent.innerHTML = `
        <div class="codex-preview-error">
          Erro ao carregar informa\xE7\xF5es do arquivo.
...
// src/main.ts
var import_meta2 = {};
var CodexPlugin = class extends import_obsidian4.Plugin {
  constructor() {
    super(...arguments);
    this.worker = null;
  ...
  async onload() {
    ...
    const ribbonIconEl = this.addRibbonIcon("file-text", "Processar PDF com Codex", (evt) => {
    ...
    this.addCommand({
      ...
      name: "Testar Conex\xE3o com Gemini",
      callback: async () => {
  ...
  showProcessingPrompt(file) {
    const notice = new import_obsidian4.Notice(`PDF detectado: ${file.name}. Clique no \xEDcone do Codex para processar.`, 5e3);
  ...
  async testGeminiConnection() {
    if (!this.settings.geminiApiKey) {
      new import_obsidian4.Notice("Configure sua chave da API Gemini primeiro nas configura\xE7\xF5es.", 5e3);
      return;
    }
    const testNotice = new import_obsidian4.Notice("Testando conex\xE3o com Gemini...", 0);
    try {
      const { GeminiClient: GeminiClient2 } = await Promise.resolve().then(() => (init_geminiClient(), geminiClient_exports));
      const isValid = await GeminiClient2.validateApiKey(this.settings.geminiApiKey);
      testNotice.hide();
      if (isValid) {
        new import_obsidian4.Notice("\u2705 Conex\xE3o com Gemini estabelecida com sucesso!", 5e3);
      } else {
        new import_obsidian4.Notice("\u274C Falha na conex\xE3o com Gemini. Verifique sua chave da API.", 5e3);
      }
    } catch (error) {
      testNotice.hide();
      console.error("Erro ao testar conex\xE3o:", error);
      new import_obsidian4.Notice("\u274C Erro ao testar conex\xE3o com Gemini.", 5e3);
    }
  }
  async processPDF(file) {
    if (!this.settings.geminiApiKey) {
      new import_obsidian4.Notice("Por favor, configure sua chave da API Gemini nas configura\xE7\xF5es do plugin.", 5e3);
      return;
    }
    try {
      const arrayBuffer = await this.app.vault.readBinary(file);
      ...
      new import_obsidian4.Notice("Erro ao ler o arquivo PDF. Verifique se o arquivo n\xE3o est\xE1 corrompido.", 5e3);
    }
  }
  async processPDFFromFile(file) {
    if (!this.settings.geminiApiKey) {
      new import_obsidian4.Notice("Por favor, configure sua chave da API Gemini nas configura\xE7\xF5es do plugin.", 5e3);
      return;
    }
    try {
      ...
      new import_obsidian4.Notice("Erro ao processar o arquivo PDF.", 5e3);
    }
  }
  async processPDFFromArrayBuffer(arrayBuffer, filename) {
    this.progressModal = new ProgressModal(this.app, () => {
      if (this.worker) {
        this.worker.terminate();
        this.worker = null;
      }
      new import_obsidian4.Notice("Processamento cancelado pelo usu\xE1rio.", 3e3);
    });
    this.progressModal.open();
    try {
      if (!this.worker) {
        this.worker = new Worker(new URL("./web-worker.ts", import_meta2.url));
      ...
      new import_obsidian4.Notice("Erro ao iniciar o processamento do PDF.", 5e3);
    }
  }
  setupWorkerListeners() {
    ...
    this.worker.onerror = (error) => {
      ...
      new import_obsidian4.Notice("Erro interno no processamento. Verifique o console para mais detalhes.", 5e3);
    };
  }
  ...
  async handleProcessingResult(result) {
    try {
      ...
      const successNotice = new import_obsidian4.Notice("", 8e3);
      const noticeEl = successNotice.noticeEl;
      noticeEl.innerHTML = `
      ...
      openButton == null ? void 0 : openButton.addEventListener("click", async () => {
        successNotice.hide();
      ...
      new import_obsidian4.Notice("Erro ao salvar a nota gerada.", 5e3);
    }
  }
